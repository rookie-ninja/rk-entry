...
// 10: DepDocPath: Path of dependencies docs generated by go list.
type BootConfigAppInfo struct {
	RK struct {
		...
		DepDocPath  string   `yaml:"depDocPath" json:"depDocPath"`
	} `yaml:"rk"`
}
...
type AppInfoEntry struct {
	...
	Dependencies       []*packages.Module `json:"-" yaml:"-"`
	dependencyFilePath string             `json:"-" yaml:"-"`
}
...
func WithDepDocPath(dependencyFilePath string) AppInfoEntryOption {
	return func(entry *AppInfoEntry) {
		entry.dependencyFilePath = dependencyFilePath
	}
}
...
func RegisterAppInfoEntriesFromConfig(configFilePath string) map[string]Entry {
	...
	entry := RegisterAppInfoEntry(
		...
		WithDepDocPath(config.RK.DepDocPath))
...
func (entry *AppInfoEntry) readAndParseDependencyFile() {
	if len(entry.dependencyFilePath) < 1 {
		return
	}

	bytes := rkcommon.MustReadFile(entry.dependencyFilePath)
	...
	for scanner.Scan() {
		line := scanner.Text()
		if line == "{" {
			str := entry.readDepBlock(scanner)
			dep := &packages.Module{}
			// ignoring error
			// TODO: Log the error
			if err := json.Unmarshal([]byte(str), dep); err != nil {
				fmt.Println(err)
			}
...
// Read line by line to get full JSON format string.
func (entry *AppInfoEntry) readDepBlock(scanner *bufio.Scanner) string {
	res := "{"
	for scanner.Scan() {
		line := scanner.Text()
		if line == "}" {
